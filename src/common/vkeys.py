...
#         hwndMain = win32gui.FindWindow("MapleStory", "test - Notepad")
# hwndChild = win32gui.GetWindow(hwndMain, win32con.GW_C
        # win32gui.SendMessage(handle, win32con.WM_KEYDOWN, win32con.VK_NEXT, 0)
        # win32gui.SendMessage(windowId, win32con.WM_KEYUP, KEY_MAP[key], 0)
        # win32api.PostMessage(windowId, win32con.WM_KEYDOWN, KEY_MAP[key], 0)
...
def key_up(key):
    ...
    if key not in KEY_MAP.keys():
        ...
        # win32gui.SendMessage(windowId, win32con.WM_KEYDOWN, KEY_MAP[key], 0)
        # win32api.PostMessage(windowId, win32con.WM_KEYUP, KEY_MAP[key], 0)
